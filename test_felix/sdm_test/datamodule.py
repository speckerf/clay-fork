...
def fill_na(arr: np.ndarray, no_data_val=NO_DATA_VALUE):
    """
    Fill the no data values in the array with mean value.
    """
    # input is a 3D array: (bands, window_size, window_size)
    # if all values are nan: impute with 0
    if np.all(arr == no_data_val):
        arr[:] = 0
        Warning("All values are no data values")
        return arr
    else:
        mean_val = np.nanmean(arr, axis=(1, 2))  # shape: (bands,)
        # input with mean
        for i in range(arr.shape[0]):
            arr[i][arr[i] == no_data_val] = mean_val[i]
    return arr
...
class Sentinel2Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (
            ...
            # TODO: change to padding with global s2 mean value
        ...
        # Fill no data values on uint16 data
        if np.any(window_data == NO_DATA_VALUE):
            window_data = fill_na(window_data)

        window_data = window_data.astype(np.float32)  # convert from uint16 to float32
...
class SDMsDataModule(pl.LightningDataModule):
    def __init__(
        self,
        raster_path,
        points_gdf,
